...
First, let's check the total migration destination per country in 2019
...
This is an interactive map, showing the main migration routes in 2019 between countries with more than 1mn migrants leaving their borders.
...
Using income-based classfication, below shows the flow of migrants between regions in 2019, as classified by the World Bank.
...
Let's visualize in more detail the biggest migration flows between the US and its top migrantion source. (greater than 1mn of migrants).
...
### Relationshiop between Migration and Wealth
...
Let's analyse the relationship between migration and three economic variables:   
...
#df_index[,c(1)]<-as.Date(as.character(df_index[,c(1)]), format = "%Y")
...
Visual inspection of GNI vs Total Migration by year and by income group.
...
Let's plot GINI index vs total exits.
...
Let's check if there's any evidence between the amount of remmitances received and total exits, as an indication of family ties, in Lower- and Low-income countries. 
...
Let's build a panel data mode consisting of data for six years (199, 1995, 2000, 2005, 2010,2015) for the three variables analysed above: GNI, Gini index and remittances received.
...
reg <- pdata.frame(df_plm, index=c(1,2), drop.index=TRUE, row.names=TRUE)
head(reg)
head(as.matrix(reg$))
```
...
There is not enough data points in the Low income group to draw any conclusion.
On the Lower-middle income group, there's an indication of indirect relationship between Gini and total of exits.
On the Upper-middle income group, it seems no relationship is present.
On the High-income group, there's an indication of direct relationship.
```{r}
